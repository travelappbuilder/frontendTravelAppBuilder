...
const ZoomResponsivePolyline = ({ positions }: any) => {
  ...
  useEffect(() => {
    const updateWeight = () => setWeight(map.getZoom());
...
const VisualisationGn: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 🔥 Convertit correctement en tableau ou tableau vide
    let agencesList: any[] = [];
    try {
      const parsed = storedAgences ? JSON.parse(storedAgences) : [];
      agencesList = Array.isArray(parsed) ? parsed : [];
    } catch {
      agencesList = [];
    }

    // 🔥 ASSURE UNE POSITION ENTREPRISE
    ...
    <div className="relative p-6 bg-white shadow-lg rounded-lg">
      ...
      <div className="relative h-[600px] w-full rounded-lg overflow-hidden shadow-lg">
        ...
        >
          ...
          {/* Marqueur entreprise */}
          <Marker position={[entrepriseData.latitude, entrepriseData.longitude]} icon={entrepriseIcon}>
            <Popup>
...
export default VisualisationGn;

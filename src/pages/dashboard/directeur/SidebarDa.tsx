...
import { FaCog, FaHome, FaUsers, FaBus, FaRoute, FaChartBar, FaEye } from "react-icons/fa";

interface Props {
  setActiveContent: (value: string) => void;
}

const SidebarDa: React.FC<Props> = ({ setActiveContent }) => {
  const [isOpen, setIsOpen] = useState(false);

  const menuItems = [
    { id: "home", label: "Tableau de bord", icon: <FaHome /> },
    { id: "gestion-gestionnaires", label: "Gestion des Gestionnaires", icon: <FaUsers /> },
    { id: "gestion-chauffeurs", label: "Gestion des Chauffeurs", icon: <FaUsers /> },
    { id: "gestion-trajets", label: "Activation des trajets", icon: <FaRoute /> },
    { id: "gestion-bus", label: "Gestion des bus", icon: <FaBus /> },
    { id: "visualisation", label: "Visualisation", icon: <FaEye /> },
          ...
          {menuItems.map((item) => (
            <button
              ...
              onClick={() => {
                ...
                setIsOpen(false); // ferme menu mobile après clic
              }}
